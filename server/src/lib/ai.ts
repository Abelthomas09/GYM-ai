...
function buildFallbackPlan(
  ...
  const splitFocus: Record<string, string[]> = {
    ...
    ppl: ["Push", "Pull", "Legs"],
    custom: ["Upper Body", "Lower Body", "Full Body"],
  };
  const focusCycle = splitFocus[profile.preferred_split] || splitFocus.custom;
  ...
  const weeklySchedule = Array.from({ length: profile.days_per_week }, (_, index) => {
    const focus = focusCycle[index % focusCycle.length];
    return {
      day: dayNames[index] || `Day ${index + 1}`,
      focus,
      exercises: workouts[focus as keyof typeof workouts].map((name) => ({
        name,
        sets: profile.experience === "beginner" ? 3 : 4,
        reps: profile.goal === "strength" ? "4-6" : "8-12",
        rest: profile.goal === "strength" ? "2-3 min" : "60-90 sec",
        rpe: profile.experience === "advanced" ? 8 : 7,
        alternatives:
          profile.equipment === "dumbbells"
            ? ["Dumbbell variation", "Bodyweight variation"]
            : ["Machine variation", "Dumbbell variation"],
      })),
    };
  });
  ...
  return {
    overview: {
      ...
      notes:
        "This plan was generated locally because the AI provider was unavailable. Keep reps controlled, stop sets with clean form, and adjust loads based on recovery.",
    },
    weeklySchedule,
    progression:
      "When you complete every set at the top of the rep range with good form, increase load by the smallest available jump next time. If recovery drops, keep the same load for another week.",
  };
}
